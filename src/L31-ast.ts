// ===========================================================
// AST type models
import { reduce, map, pipe, zipWith } from "ramda";
...
;;         | (cond <cond-claus>* <else-clause>)   // condExp((cond-clauses:List(cond-clause), else:else-clause):
...
;;                (<sexp>+ . <sexp>) | ( <sexp>* )       ##### L31
;; <cond-clause> =:: (<cexp> <cexp>) // cond-clause(test:cexp, then:cexp)
;; <else-clause> =:: (else <cexp>) // else-clause(else:cexp)
*/

export type Exp = DefineExp | CExp;
export type AtomicExp = NumExp | BoolExp | StrExp | PrimOp | VarRef;
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | CondExp| CondClauseExp | ElseClauseExp;
export type CExp =  AtomicExp | CompoundExp;
export type Clause = CondClauseExp | ElseClauseExp;
...
// L3.1
export type CondExp = { tag: "CondExp"; CondClauses: CondClauseExp[]; Else: ElseClauseExp }
export type CondClauseExp = { tag: "CondClauseExp"; test: CExp; then: CExp }
export type ElseClauseExp = { tag: "ElseClauseExp"; res: CExp }
...
// L3.1
export const makeCondExp = (CondClauses: CondClauseExp[], Else: ElseClauseExp): CondExp =>
({ tag: "CondExp", CondClauses: CondClauses, Else: Else })
export const makeCondClauseExp = (test: CExp, then: CExp): CondClauseExp =>
({ tag: "CondClauseExp", test: test, then: then })
export const makeElseClauseExp = (res: CExp): ElseClauseExp =>
({ tag: "ElseClauseExp", res: res })
...
export const isCondClauseExp = (x: any): x is CondClauseExp => x.tag === "CondClauseExp";
export const isElseClauseExp = (x: any): x is ElseClauseExp => x.tag === "ElseClauseExp";
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isCondExp(x) || isCondClauseExp(x) || isElseClauseExp(x);
...
const parseCondExp = (params: Sexp[]): Result<CondExp> =>
    isEmpty(params) ? makeFailure("Empty args for cond") :
    bind(mapResult(parseClause, params), parseGoodCond);

const parseClause = (params: Sexp): Result<Clause> =>
    params.length !== 2 ? makeFailure("not 2 parameters in the clause") :
    isEmpty(params) ? makeFailure("Empty args for clause not allowed") :
    isToken(params) ? makeFailure("clause cant be a token") :
    params[0] === "else" ? parseElseExp(params[1]) :
    parseCondClauseExp(params);

const parseElseExp = (params: Sexp): Result<ElseClauseExp> =>
    bind(parseL31CExp(params), (cexp: CExp) => makeOk(makeElseClauseExp(cexp)));

const parseCondClauseExp = (params: Sexp[]): Result<CondClauseExp> =>
    params.length !== 2 ? makeFailure(`Expression not of the form (<cexp> <cexp>): ${JSON.stringify(params, null, 2)}`) :
    mapv(mapResult(parseL31CExp, params), (cexps: CExp[]) =>
        makeCondClauseExp(cexps[0], cexps[1]));

const parseGoodCond = (params: Clause[]): Result<CondExp> =>
    params.length <= 2 ? makeFailure("not enough parameters in the cond") :
    !isElseClauseExp(params[params.length - 1]) ? makeFailure("else is not the last parameter") :
    checkForElse(params.slice(0,params.length-1)) ? makeFailure("there is an else out of place") : //need to make sure slicing is correct
    makeOk(makeCondExp(params.slice(0,params.length-1) as CondClauseExp[], params[params.length-1] as ElseClauseExp));

const checkForElse = (params: Clause[]): boolean =>
    reduce(((acc: boolean, curr: Clause) => acc || isElseClauseExp(curr)), false, params);
...
//import { CondClauseExp } from "../imp/L3-ast";
...
const unparseCondClauses = (clauses: CondClauseExp[]) : string =>
    `${map((clause: CondClauseExp) => `(${unparseL31(clause.test)} ${unparseL31(clause.then)})`, clauses).join(" ")}` 


const unparseCondExp = (condExp: CondExp) : string =>
    `(cond ${unparseCondClauses(condExp.CondClauses)} (else ${unparseL31(condExp.Else.res)}))`
...
export const unparseL31 = (exp: Program | Exp): string =>
    ...
    isCondClauseExp(exp) ? "ERROR: CondClauseExp" :
    isElseClauseExp(exp) ? "ERROR: ElseClauseExp" :
